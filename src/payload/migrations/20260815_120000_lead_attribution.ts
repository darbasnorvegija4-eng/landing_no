...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.execute(
    sql`ALTER TABLE "leads" ADD COLUMN IF NOT EXISTS "gbraid" varchar`,
  );
  await db.execute(
    sql`ALTER TABLE "leads" ADD COLUMN IF NOT EXISTS "wbraid" varchar`,
  );
  await db.execute(
    sql`ALTER TABLE "leads" ADD COLUMN IF NOT EXISTS "fbclid" varchar`,
  );
  await db.execute(
    sql`ALTER TABLE "leads" ADD COLUMN IF NOT EXISTS "msclkid" varchar`,
  );
  await db.execute(
  ...
  await db.execute(
    sql`ALTER TABLE "leads" ADD COLUMN IF NOT EXISTS "marketing_consent" varchar`,
  );
}
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  await db.execute(sql`ALTER TABLE "leads" DROP COLUMN IF EXISTS "gbraid"`);
  await db.execute(sql`ALTER TABLE "leads" DROP COLUMN IF EXISTS "wbraid"`);
  await db.execute(sql`ALTER TABLE "leads" DROP COLUMN IF EXISTS "fbclid"`);
  await db.execute(sql`ALTER TABLE "leads" DROP COLUMN IF EXISTS "msclkid"`);
  await db.execute(
  ...
  await db.execute(
    sql`ALTER TABLE "leads" DROP COLUMN IF EXISTS "marketing_consent"`,
  );
}
